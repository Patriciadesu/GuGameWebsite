...
(0, node_test_1.default)('player mutations share the active level-matched topic skill-map guard', () => {
    ...
    strict_1.default.ok(guardCalls.length >= 4, 'unlock, step completion, request, and approval must use the shared guard');
});
(0, node_test_1.default)('connection writes reject self, duplicate, missing, and cross-map targets', () => {
    ...
    strict_1.default.ok(guardCalls.length >= 3, 'bulk, create, and update connection writes must use the shared guard');
});
(0, node_test_1.default)('privileged routes revalidate Discord roles with a short-lived cache and fail closed', () => {
    strict_1.default.match(serverSource, /discordRoleCache = new KeyedAsyncTtlCache[^\n]*5 \* 60_000/);
    strict_1.default.match(serverSource, /const refreshPrivilegedRole = async/);
    strict_1.default.match(serverSource, /fetchUserRoleAndNickname\(storedUser\.accessToken!/);
    strict_1.default.match(serverSource, /Role verification unavailable/);
    strict_1.default.match(serverSource, /await refreshPrivilegedRole\(req\)/);
});

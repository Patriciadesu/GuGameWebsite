...
const SkillTreeSettings_1 = __importDefault(require("./models/SkillTreeSettings"));
...
const OfficeQuestCache_1 = __importDefault(require("./models/OfficeQuestCache"));
const voiceTracker_1 = require("./services/voiceTracker");
const officeCatalog_1 = require("./services/officeCatalog");
const officeQuestCatalog_1 = require("./services/officeQuestCatalog");
const asyncCache_1 = require("./services/asyncCache");
const cachedSessionStore_1 = require("./services/cachedSessionStore");
const keyedBatchLoader_1 = require("./services/keyedBatchLoader");
...
const fictionService_1 = require("./services/fictionService");
const hamsterQuest_1 = require("./services/hamsterQuest");
...
const isAllowedClientOrigin = (origin, requestOrigin) => {
    ...
    if (normalizedOrigin === FRONTEND_ORIGIN || normalizedOrigin === requestOrigin)
        return true;
...
const hamsterInventorySyncCache = new asyncCache_1.KeyedAsyncTtlCache(30000, 2000);
const userInventoryCache = new asyncCache_1.KeyedAsyncTtlCache(15000, 2000);
const inventoryShopItemsCache = new asyncCache_1.AsyncTtlCache(30000);
const inventoryPurchasesLoader = new keyedBatchLoader_1.KeyedBatchLoader(async (discordIds) => {
    const purchases = await Purchase_1.default.find({
        userId: { $in: discordIds },
        $or: [{ quantity: { $gt: 0 } }, { quantity: { $exists: false } }]
    }).sort({ updatedAt: -1 }).lean();
    const purchasesByUserId = new Map(discordIds.map(discordId => [discordId, []]));
...
const getInventoryShopItemsById = () => inventoryShopItemsCache.get(async () => {
    const items = await ShopItem_1.default.find({}).lean();
...
const getUserSummariesByDiscordId = async (discordIds) => {
    const uniqueIds = [...new Set(discordIds)];
    if (uniqueIds.length === 0)
        return new Map();
    const users = await User_1.default.find({ discordId: { $in: uniqueIds } })
...
const presentShopItems = async (items) => {
    ...
    try {
        const catalogItems = await (0, officeCatalog_1.getOfficeCatalogItems)();
...
const performHamsterQuestInventorySync = async (discordId) => {
    if (!(0, hamsterQuest_1.isHamsterQuestConfigured)()) {
        throw new Error('HamsterQuest integration is not configured');
    }
    const [remoteInventory, catalogItems] = await Promise.all([
        (0, hamsterQuest_1.getHamsterQuestInventory)(discordId),
        (0, officeCatalog_1.getOfficeCatalogItems)().catch(() => [])
    ]);
    ...
    if (remoteByItemId.size > 0) {
        try {
            await ShopItem_1.default.bulkWrite([...remoteByItemId].map(([externalItemId, remoteEntry]) => {
    ...
    const existingExternalItems = await ShopItem_1.default.find({
    ...
    if (purchaseOperations.length > 0) {
        await Purchase_1.default.bulkWrite(purchaseOperations, { ordered: false });
    }
    userInventoryCache.delete(discordId);
    return remoteInventory;
};
const syncHamsterQuestInventory = async (discordId, forceRefresh = false) => {
    if (forceRefresh)
        hamsterInventorySyncCache.delete(discordId);
    return hamsterInventorySyncCache.get(discordId, () => performHamsterQuestInventorySync(discordId));
};
const loadPresentedUserInventory = async (discordId, resolveCatalog) => {
...
const presentUserInventory = (discordId, resolveCatalog = true) => {
    if (resolveCatalog)
        return loadPresentedUserInventory(discordId, true);
    return userInventoryCache.get(discordId, () => loadPresentedUserInventory(discordId, false));
};
const getShopGuildScope = async (availableToAllGuilds, guildIds) => {
    if (availableToAllGuilds !== false) {
        return { availableToAllGuilds: true, guildIds: [] };
    }
    if (!Array.isArray(guildIds)) {
        throw new Error('Select at least one guild or make the item available to all guilds');
    }
    const selectedGuildIds = [...new Set(guildIds.filter((guildId) => typeof guildId === 'string' && guildId.trim().length > 0))];
    if (selectedGuildIds.length === 0) {
        throw new Error('Select at least one guild or make the item available to all guilds');
    }
    const validGuilds = await Guild_1.default.find({ _id: { $in: selectedGuildIds } }).select('_id').lean();
...
const isShopItemAvailableToUser = (shopItem, user) => shopItem.availableToAllGuilds !== false || Boolean(user.guildId && shopItem.guildIds?.includes(user.guildId));
const activeSkillsCache = new asyncCache_1.AsyncTtlCache(30000);
const constellationMapPageCache = new asyncCache_1.KeyedAsyncTtlCache(30000, 500);
const progressionCache = new asyncCache_1.AsyncTtlCache(10000);
const sessionUserCache = new asyncCache_1.KeyedAsyncTtlCache(15000, 2000);
const discordRoleCache = new asyncCache_1.KeyedAsyncTtlCache(5 * 60000, 2000);
const getActiveSkills = () => activeSkillsCache.get(() => Skill_1.default.find({ isActive: true }).sort({ layer: 1, position: 1 }).lean());
const encodeConstellationCursor = (displayOrder, id) => Buffer.from(JSON.stringify({ displayOrder, id }), 'utf8').toString('base64url');
const decodeConstellationCursor = (cursor) => {
    try {
        const parsed = JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8'));
        if (!Number.isFinite(parsed.displayOrder) || !mongoose_1.default.Types.ObjectId.isValid(parsed.id)) {
            throw new Error('Invalid cursor values');
        }
        return { displayOrder: Number(parsed.displayOrder), id: String(parsed.id) };
    }
    catch {
        throw new constellationService_1.ConstellationOperationError('cursor is invalid');
    }
};
const getConstellationMapPage = (options) => {
    ...
    return constellationMapPageCache.get(cacheKey, async () => {
        ...
        const rows = await ConstellationMap_1.default.find(query)
...
const presentSkillsForUser = (skills, user) => {
...
const filterSkillsForUserLevel = async (skills, userLevel) => {
    const mapIds = [...new Set(skills.map(skill => skill.constellationMapId?.toString()).filter(Boolean))];
    if (mapIds.length === 0)
        return skills;
    const maps = await ConstellationMap_1.default.find({ _id: { $in: mapIds } })
        .select('_id scope level')
        .lean();
    const mapsById = new Map(maps.map(map => [map._id.toString(), map]));
    const allowedTopicMaps = await ConstellationMap_1.default.find({
        scope: 'topic',
        isActive: true,
...
const getProgressionSnapshot = () => progressionCache.get(async () => {
    const [users, guilds, skills, pendingApprovals] = await Promise.all([
        User_1.default.find().select('discordId username nickname avatar guildId unlockedSkills').lean(),
        Guild_1.default.find().select('name assetPointName').lean(),
        getActiveSkills(),
        ApprovalRequest_1.default.find({ status: 'pending' }).select('userId skillId').lean()
    ]);
    const progressSkillIds = new Set(skills
    ...
    const compareProgress = (a, b) => b.progress - a.progress || a.name.localeCompare(b.name);
...
const presentProgressionLeaderboard = (snapshot, currentUserId, currentGuildId) => {
...
const deleteSkillsWithReferences = async (skillObjectIds) => {
    ...
    await Promise.all([
        ...
        ApprovalRequest_1.default.deleteMany({ skillId: { $in: skillIds } })
    ]);
    const result = await Skill_1.default.deleteMany({ _id: { $in: skillObjectIds } });
    sessionUserCache.clear();
    invalidateProgressionCache();
    invalidateSkillCaches();
    return result.deletedCount;
};
const collectConstellationMapIds = async (rootMapId) => {
    const collected = [rootMapId];
    let frontier = [rootMapId];
    while (frontier.length > 0) {
        const children = await ConstellationMap_1.default.find({ parentMapId: { $in: frontier } }).select('_id').lean();
        frontier = children.map(child => child._id);
        collected.push(...frontier);
    }
    return collected;
};
const sendConstellationError = (res, error, fallbackMessage) => {
    if (error instanceof constellationService_1.ConstellationOperationError) {
...
const getPlayerEligibleSkill = async (skillId, userLevel) => {
    const skill = await Skill_1.default.findOne({ _id: skillId, isActive: true });
    if (!skill?.constellationMapId)
        return null;
    const map = await ConstellationMap_1.default.exists({
...
const assertValidConnectionTargets = async (sourceSkillId, sourceMapId, connections) => {
    if (!Array.isArray(connections)) {
        throw new constellationService_1.ConstellationOperationError('Connections must be an array');
    }
    if (!sourceMapId) {
        throw new constellationService_1.ConstellationOperationError('Connected stars must belong to a constellation map', 409);
    }
    const targetIds = connections.map(connection => String(connection?.targetSkillId || ''));
    if (targetIds.some(targetId => !mongoose_1.default.Types.ObjectId.isValid(targetId))) {
        throw new constellationService_1.ConstellationOperationError('Every connection target must be a valid star ID');
    }
    if (targetIds.includes(sourceSkillId)) {
        throw new constellationService_1.ConstellationOperationError('A star cannot connect to itself');
    }
    if (new Set(targetIds).size !== targetIds.length) {
        throw new constellationService_1.ConstellationOperationError('A star cannot contain duplicate connections');
    }
    if (targetIds.length === 0)
        return;
    const matchingTargets = await Skill_1.default.countDocuments({
        _id: { $in: targetIds },
        constellationMapId: sourceMapId
    });
    if (matchingTargets !== targetIds.length) {
        throw new constellationService_1.ConstellationOperationError('Connection targets must exist in the same constellation map', 409);
    }
};
...
const applyConstellationMapFields = (map, body) => {
...
const assertValidLevel = (value, fieldName = 'level') => {
    if (!Number.isInteger(value) || Number(value) < 1) {
        throw new constellationService_1.ConstellationOperationError(`${fieldName} must be a positive integer`);
    }
};
...
async function fetchUserRoleAndNickname(accessToken, userId) {
    // Fetch the user's guild member information
    const response = await axios_1.default.get(`https://discord.com/api/v10/users/@me/guilds/${ADMIN_GUILD_ID}/member`, {
        headers: {
...
async function checkUserRoleAndNickname(accessToken, userId) {
    try {
...
// MongoDB connection
mongoose_1.default.connect(MONGODB_URI, MONGODB_DB_NAME ? { dbName: MONGODB_DB_NAME } : undefined)
    .then(() => {
    ...
    if (DISCORD_BOT_TOKEN && ADMIN_GUILD_ID) {
        ...
        voiceTracker = new voiceTracker_1.VoiceTracker(ADMIN_GUILD_ID);
...
const persistentSessionStore = connect_mongo_1.default.create({
...
app.use((0, express_session_1.default)({
    ...
    store: new cachedSessionStore_1.CachedSessionStore(persistentSessionStore),
    cookie: {
...
app.use((req, res, next) => {
    if (!unsafeMethods.has(req.method) || !req.isAuthenticated())
        return next();
...
passport_1.default.use(new passport_discord_1.Strategy({
    ...
    try {
        ...
        if ((0, hamsterQuest_1.isHamsterQuestConfigured)()) {
            try {
                await (0, hamsterQuest_1.ensureHamsterQuestUser)(user.discordId);
            }
            catch (error) {
                console.error(`Unable to sync HamsterQuest account for Discord user ${user.discordId}:`, (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error));
            }
        }
...
passport_1.default.deserializeUser(async (id, done) => {
    try {
        const user = await sessionUserCache.get(id, async () => {
            const storedUser = await User_1.default.findOne({ discordId: id })
...
const refreshPrivilegedRole = async (req) => {
    if (!req.user || (req.user.role !== 'admin' && req.user.role !== 'super-admin'))
        return req.user?.role || 'user';
    const storedUser = await User_1.default.findOne({ discordId: req.user.id }).select('accessToken role').lean();
    if (!storedUser?.accessToken) {
        if (process.env.NODE_ENV !== 'production')
            return req.user.role;
        throw new Error('Discord access token is unavailable for role verification');
    }
    const role = await discordRoleCache.get(req.user.id, async () => (await fetchUserRoleAndNickname(storedUser.accessToken, req.user.id)).role);
    if (storedUser.role !== role) {
        await User_1.default.updateOne({ discordId: req.user.id }, { $set: { role, isAdmin: role === 'admin' || role === 'super-admin' } });
...
const requireConstellationSkillEditor = async (req, res, next) => {
    ...
    try {
        ...
        if (refreshedRole === 'super-admin')
            return next();
        const skillId = req.params.id;
        if (!skillId) {
            const constellationMapId = String(req.body.constellationMapId || '');
            if (!mongoose_1.default.Types.ObjectId.isValid(constellationMapId)) {
                return res.status(403).json({ error: 'Forbidden', message: 'Admins can only create Constellation stars' });
            }
            const mapExists = await ConstellationMap_1.default.exists({ _id: constellationMapId });
        ...
        const constellationSkillExists = await Skill_1.default.exists({ _id: skillId, constellationMapId: { $exists: true } });
...
app.get('/api/auth/discord/callback', passport_1.default.authenticate('discord', {
...
const presentMainMenuUser = (user, assetPointName) => ({
    discordId: user.id,
...
const loadMainMenuState = async (user, includeInitialData) => {
    ...
    const commonState = {
        ...
        progressionLeaderboard: presentProgressionLeaderboard(progressionSnapshot, user.id, guildId)
    };
    ...
    return {
        ...
        inventory: {
            ...
            hamsterQuestConfigured: (0, hamsterQuest_1.isHamsterQuestConfigured)(),
            syncWarning: null
        }
    };
};
app.get('/api/mainmenu/bootstrap', requireAuth, async (req, res) => {
    try {
...
app.get('/api/mainmenu/status', requireAuth, async (req, res) => {
    try {
...
if (process.env.NODE_ENV !== 'production') {
    ...
    app.post('/api/auth/test-login', async (req, res) => {
        ...
        const hasValidTestKey = Boolean(TEST_BYPASS_KEY &&
            suppliedKeyBuffer.length === expectedKeyBuffer.length &&
            crypto_1.default.timingSafeEqual(suppliedKeyBuffer, expectedKeyBuffer));
        ...
        try {
            ...
            const testUser = requestedUserId
                ? await User_1.default.findOne({ discordId: requestedUserId })
                : await User_1.default.findOne({ role: 'super-admin' }) || await User_1.default.findOne({ role: 'admin' });
            if (!testUser) {
...
app.get('/api/super-admin/settings', requireSuperAdmin, (req, res) => {
    res.json({
        success: true,
...
app.post('/api/guilds', requireSuperAdmin, async (req, res) => {
    try {
        ...
        const existingGuild = await Guild_1.default.findOne({ name });
        if (existingGuild) {
        ...
        if (leaderIdsArray.length > 0) {
            for (const leaderId of leaderIdsArray) {
                const leader = await User_1.default.findOne({ discordId: leaderId });
                if (!leader) {
        ...
        const guild = await Guild_1.default.create({
            name,
...
app.get('/api/guilds', async (req, res) => {
    try {
        const guilds = await Guild_1.default.find().sort({ createdAt: -1 });
...
app.get('/api/guilds/:id', requireAdmin, async (req, res) => {
    try {
        const guild = await Guild_1.default.findById(req.params.id);
        if (!guild) {
...
app.put('/api/guilds/:id', requireSuperAdmin, async (req, res) => {
    try {
        ...
        if (adminIds !== undefined)
            guild.adminIds = adminIds;
        if (assetPointName !== undefined)
            guild.assetPointName = assetPointName;
        await guild.save();
        invalidateProgressionCache();
        res.json({ success: true, guild });
    }
...
app.delete('/api/guilds/:id', requireSuperAdmin, async (req, res) => {
    try {
        const guild = await Guild_1.default.findByIdAndDelete(req.params.id);
        if (!guild) {
            return res.status(404).json({ error: 'Guild not found' });
        }
        // Remove guild association from all users
        await User_1.default.updateMany({ guildId: req.params.id }, { $unset: { guildId: '' } });
        sessionUserCache.clear();
        invalidateProgressionCache();
...
app.get('/api/guilds/:id/members', requireAdmin, async (req, res) => {
    try {
        const members = await User_1.default.find({ guildId: req.params.id }).select('-accessToken -refreshToken');
...
app.get('/api/guilds/:id/stats', requireAdmin, async (req, res) => {
    try {
        const guild = await Guild_1.default.findById(req.params.id);
        if (!guild) {
            return res.status(404).json({ error: 'Guild not found' });
        ...
        if (guild.guildLeaderIds && guild.guildLeaderIds.length > 0) {
            for (const leaderId of guild.guildLeaderIds) {
                const leader = await User_1.default.findOne({ discordId: leaderId }).select('-accessToken -refreshToken');
                if (leader) {
                    guildLeaders.push({
...
app.get('/api/user/guild-info', requireAdmin, async (req, res) => {
    try {
        // Find guilds where user is one of the leaders
        const leaderGuilds = await Guild_1.default.find({ guildLeaderIds: req.user.id });
        ...
        const members = await User_1.default.find({ guildId: guild._id }).select('-accessToken -refreshToken');
        const totalMembers = members.length;
        const totalAssetPoints = members.reduce((sum, member) => sum + member.assetPoints, 0);
        res.json({
            success: true,
...
app.get('/api/users/:userId', requireAuth, async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin' || req.user.role === 'super-admin') {
            ...
            targetUser = await User_1.default.findOne({ discordId: requestingUserId });
        }
        if (!targetUser) {
            return res.status(404).json({ error: 'User not found' });
        }
...
app.get('/api/users', requireAdmin, async (req, res) => {
    try {
        const users = await User_1.default.find().select('-accessToken -refreshToken').sort({ username: 1 });
...
app.post('/api/users/:userId/guild', async (req, res) => {
    try {
        ...
        const requestingUser = await User_1.default.findOne({ discordId: requestingUserId });
        if (!requestingUser) {
        ...
        const user = await User_1.default.findOne({ discordId: targetUserId });
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        // Verify guild exists
        if (guildId) {
            const guild = await Guild_1.default.findById(guildId);
            if (!guild) {
                return res.status(404).json({ error: 'Guild not found' });
            }
        }
...
app.post('/api/users/:userId/asset-points', requireAdmin, async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOne({ discordId: req.params.userId });
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        if (operation === 'add') {
...
app.patch('/api/users/:userId/level', requireAdmin, async (req, res) => {
    try {
...
const presentOfficeQuestCatalog = async () => {
    const [quests, importedSkills] = await Promise.all([
        OfficeQuestCache_1.default.find({}).sort({ title: 1 }).lean(),
        Skill_1.default.find({ externalSource: 'office-quest' }).select('externalQuestId').lean()
    ]);
...
const normalizeDescriptionParts = (value) => Array.isArray(value)
    ? value.filter((part) => part && typeof part.content === 'string' && part.content.trim()).slice(0, 100)
        .map((part) => ({ type: typeof part.type === 'string' ? part.type.slice(0, 40) : 'Text', content: part.content.trim().slice(0, 100000) }))
    : [];
const normalizeSubQuests = (value, persistedSteps = []) => {
    if (!Array.isArray(value))
        return [];
    const steps = value
        .filter((subQuest) => subQuest && typeof subQuest.title === 'string' && subQuest.title.trim())
        .slice(0, 100)
        .map((subQuest) => ({
    ...
    return (0, questStepNormalization_1.normalizeQuestStepExternalIds)(steps, persistedSteps);
};
const validControlPoints = (value) => Array.isArray(value) && value.length === 2 && value.every(point => point && Number.isFinite(point.x) && Number.isFinite(point.y));
app.get('/api/admin/star-master/tags', requireAdmin, async (_req, res) => {
    try {
        const tags = await (0, officeQuestCatalog_1.getOfficeQuestTags)();
        res.json({
            success: true,
            tags: tags
...
app.get(['/api/admin/star-master/quests', '/api/admin/hamquest/quests'], requireAdmin, async (req, res) => {
    try {
        ...
        const [{ quests, pagination }, importedSkills] = await Promise.all([
            (0, officeQuestCatalog_1.getOfficeQuestPage)({
                page,
                limit,
                ...(search ? { search } : {}),
                ...(type ? { type } : {}),
            ...
            Skill_1.default.find({ externalQuestId: { $exists: true } }).select('externalQuestId constellationMapId').lean()
        ]);
        ...
        res.json({
            ...
            quests: quests.map(quest => ({
                ...
                type: quest.type,
                description: (0, officeQuestCatalog_1.getOfficeQuestDescription)(quest.description),
                imageUrl: (0, officeQuestCatalog_1.getOfficeQuestImageUrl)(quest),
...
const starMasterSkillPayload = (quest, map, constellationMapId, mapSkillIndex, totalSkillIndex) => {
    const columns = 3;
    const column = mapSkillIndex % columns;
    const row = Math.floor(mapSkillIndex / columns);
    const description = (0, officeQuestCatalog_1.getOfficeQuestDescription)(quest.description) || 'Imported from StarMaster.';
    const imageUrl = (0, officeQuestCatalog_1.getOfficeQuestImageUrl)(quest);
    const outcomes = (quest.subQuests || []).map((subQuest) => subQuest.title?.trim()).filter(Boolean).slice(0, 4);
    return {
        title: quest.title || 'Untitled quest',
        description,
        cost: 0,
        ...
        subQuests: normalizeSubQuests((quest.subQuests || []).map((subQuest) => ({
            ...
            description: (0, officeQuestCatalog_1.getOfficeQuestDescription)(subQuest?.description),
            descriptionParts: (0, officeQuestCatalog_1.getOfficeQuestDescriptionParts)(subQuest?.description),
            type: subQuest?.subQuestType
        }))),
        isActive: true
    };
};
const getStarMasterImportMap = async (constellationMapId) => {
    if (!mongoose_1.default.Types.ObjectId.isValid(constellationMapId)) {
        throw Object.assign(new Error('Select a Topic Constellation before importing'), { status: 400 });
    }
    const map = await ConstellationMap_1.default.findById(constellationMapId).lean();
    if (!map)
        throw Object.assign(new Error('Topic Constellation not found'), { status: 404 });
    if (map.scope !== 'topic') {
        throw Object.assign(new Error('StarMaster quests can only be imported into a Topic Constellation'), { status: 400 });
    }
    return map;
};
app.post('/api/admin/star-master/quests/import', requireAdmin, async (req, res) => {
    const externalQuestIds = Array.isArray(req.body.externalQuestIds)
        ? [...new Set(req.body.externalQuestIds
                .filter((value) => typeof value === 'string')
                .map(value => value.trim())
                .filter(Boolean))]
        : [];
    if (externalQuestIds.length === 0)
        return res.status(400).json({ error: 'Select at least one quest to import' });
    if (externalQuestIds.length > 50)
        return res.status(400).json({ error: 'Import up to 50 quests at a time' });
    try {
        const constellationMapId = String(req.body.constellationMapId || '');
        const map = await getStarMasterImportMap(constellationMapId);
        const [mapSkillCount, totalSkillCount] = await Promise.all([
            Skill_1.default.countDocuments({ constellationMapId }),
        ...
        const importNext = async () => {
            while (cursor < externalQuestIds.length) {
                ...
                try {
                    const quest = await (0, officeQuestCatalog_1.getOfficeQuestById)(externalQuestId);
                    const skill = await Skill_1.default.create(starMasterSkillPayload(quest, map, constellationMapId, mapSkillCount + itemIndex, totalSkillCount + itemIndex));
                    imported.push(skill);
                }
                catch (error) {
                    failed.push({
...
app.post(['/api/admin/star-master/quests/:externalQuestId/import', '/api/admin/hamquest/quests/:externalQuestId/import'], requireAdmin, async (req, res) => {
    try {
        const constellationMapId = String(req.body.constellationMapId || '');
        const map = await getStarMasterImportMap(constellationMapId);
        const [quest, mapSkillCount, totalSkillCount] = await Promise.all([
            (0, officeQuestCatalog_1.getOfficeQuestById)(req.params.externalQuestId),
            Skill_1.default.countDocuments({ constellationMapId }),
            Skill_1.default.countDocuments()
        ]);
        const skill = await Skill_1.default.create(starMasterSkillPayload(quest, map, constellationMapId, mapSkillCount, totalSkillCount));
        invalidateSkillCaches();
...
app.get('/api/admin/office-quest-catalog', requireAdmin, async (_req, res) => {
    try {
        const items = await presentOfficeQuestCatalog();
        const latestSync = await OfficeQuestCache_1.default.findOne({}).sort({ syncedAt: -1 }).select('syncedAt').lean();
...
app.post('/api/admin/office-quest-catalog/sync', requireAdmin, async (_req, res) => {
    try {
        const sourceQuests = await (0, officeQuestCatalog_1.getOfficeQuests)();
        const seenDetails = new Set();
        const syncedAt = new Date();
        const uniqueQuests = sourceQuests.filter(quest => {
            if (!quest?._id || !quest.title?.trim())
                return false;
            const detailHash = (0, officeQuestCatalog_1.getOfficeQuestDetailHash)(quest);
            if (seenDetails.has(detailHash))
                return false;
                ...
                updateOne: {
                    ...
                    update: {
                        $set: {
                            ...
                            detailHash: (0, officeQuestCatalog_1.getOfficeQuestDetailHash)(quest),
                            syncedAt
                        }
                    },
                    upsert: true
        ...
        if (syncOperations.length > 0)
            await OfficeQuestCache_1.default.bulkWrite(syncOperations);
        await OfficeQuestCache_1.default.deleteMany({ externalId: { $nin: uniqueQuests.map(quest => quest._id) } });
        const importedQuestSteps = uniqueQuests.map(quest => ({
            updateOne: {
                filter: { externalSource: 'office-quest', externalQuestId: quest._id },
                update: {
                    $set: {
                        subQuests: normalizeSubQuests((quest.subQuests || []).map(subQuest => ({
                            externalId: subQuest?._id,
        ...
        if (importedQuestSteps.length > 0)
            await Skill_1.default.bulkWrite(importedQuestSteps);
...
app.post('/api/admin/office-quest-catalog/:externalQuestId/import', requireAdmin, async (req, res) => {
    try {
        const quest = await OfficeQuestCache_1.default.findOne({ externalId: req.params.externalQuestId });
        if (!quest)
            return res.status(404).json({ error: 'Quest is not in the cached catalog. Sync it first.' });
        const existing = await Skill_1.default.findOne({ externalSource: 'office-quest', externalQuestId: quest.externalId });
        if (existing)
            return res.status(409).json({ error: 'This Office quest has already been imported' });
...
app.post('/api/admin/office-quest-catalog/:externalQuestId/reimport', requireAdmin, async (req, res) => {
    try {
        const quest = await OfficeQuestCache_1.default.findOne({ externalId: req.params.externalQuestId });
        if (!quest)
            return res.status(404).json({ error: 'Quest is not in the cached catalog. Sync it first.' });
        const skill = await Skill_1.default.findOne({ externalSource: 'office-quest', externalQuestId: quest.externalId });
        if (!skill)
            return res.status(404).json({ error: 'This Office quest has not been imported yet.' });
...
app.get('/api/admin/quest-tree/json', requireSuperAdmin, async (_req, res) => {
    try {
        const skills = await Skill_1.default.find({}).sort({ createdAt: 1 });
        res.json({
            version: 1,
...
app.post('/api/admin/quest-tree/import', requireSuperAdmin, async (req, res) => {
    try {
        ...
        const keys = quests.map((quest, index) => String(quest.key || `quest-${index + 1}`));
        ...
        for (let index = 0; index < quests.length; index++) {
            ...
            const references = [
                ...
                ...(Array.isArray(quest.connections) ? quest.connections.map((connection) => connection.targetKey) : [])
        ...
        const created = await Skill_1.default.insertMany(quests.map((quest, index) => new Skill_1.default({
            title: String(quest.title).trim(),
        ...
        await Promise.all(created.map(async (skill, index) => {
            ...
            skill.prerequisites = (Array.isArray(quest.prerequisites) ? quest.prerequisites : [])
                .map((key) => importedIdByKey.get(String(key)))
                .filter(Boolean);
            skill.connections = (Array.isArray(quest.connections) ? quest.connections : []).map((connection) => ({
            ...
            })).filter((connection) => connection.targetSkillId);
            await skill.save();
        }));
        invalidateSkillCaches();
...
app.get('/api/constellation-maps', requireAuth, async (req, res) => {
    try {
        ...
        if (parentMapId && !mongoose_1.default.Types.ObjectId.isValid(parentMapId)) {
            return res.status(400).json({ error: 'parentMapId must be a valid ID' });
        }
        if (gatewaySkillId && !mongoose_1.default.Types.ObjectId.isValid(gatewaySkillId)) {
        ...
        const visibleMaps = isAdmin
            ? page.maps
            : page.maps.filter(map => map.scope === 'discipline' || ((map.level || 1) >= (req.user.state?.level || 1) &&
...
app.get('/api/constellation-maps/:id', requireAuth, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid constellation map ID' });
        }
        const isAdmin = req.user.role === 'admin' || req.user.role === 'super-admin';
        const map = await ConstellationMap_1.default.findOne({
            _id: req.params.id,
            ...(isAdmin ? {} : {
                isActive: true,
        ...
        if (!map)
            return res.status(404).json({ error: 'Constellation map not found' });
        ...
        if (!isAdmin && map.scope === 'discipline') {
            const visibleTopicMaps = await ConstellationMap_1.default.find({
                parentMapId: map._id,
                scope: 'topic',
                isActive: true,
                level: {
...
app.post('/api/constellation-maps', requireAdmin, async (req, res) => {
    try {
        const map = new ConstellationMap_1.default();
        applyConstellationMapFields(map, req.body);
        if (map.scope === 'topic')
            assertValidLevel(map.level || 1, 'Topic level');
        else
            map.level = 1;
        if (!Object.prototype.hasOwnProperty.call(req.body, 'isActive'))
            map.isActive = false;
        await (0, constellationService_1.validateConstellationMapLinkage)({
...
app.patch('/api/constellation-maps/:id', requireAdmin, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid constellation map ID' });
        }
        const map = await ConstellationMap_1.default.findById(req.params.id);
        if (!map)
            return res.status(404).json({ error: 'Constellation map not found' });
        applyConstellationMapFields(map, req.body);
        if (map.scope === 'topic')
            ...
            map.level = 1;
        await (0, constellationService_1.validateConstellationMapContents)(map._id.toString(), map.scope);
        await (0, constellationService_1.validateConstellationMapLinkage)({
            constellationType: map.constellationType,
            scope: map.scope,
            parentMapId: map.parentMapId,
...
app.patch('/api/constellation-maps/:id/layout', requireAdmin, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid constellation map ID' });
        }
        const map = await ConstellationMap_1.default.findById(req.params.id).select('_id viewport').lean();
        if (!map)
            return res.status(404).json({ error: 'Constellation map not found' });
        const nodes = (0, constellationService_1.normalizeConstellationLayout)(req.body?.nodes, map.viewport);
        const matchingSkillCount = await Skill_1.default.countDocuments({
            _id: { $in: nodes.map(node => node.skillId) },
            constellationMapId: map._id
        });
        if (matchingSkillCount !== nodes.length) {
            throw new constellationService_1.ConstellationOperationError('Every layout node must belong to this constellation map', 409);
        }
        const result = await Skill_1.default.bulkWrite(nodes.map(node => ({
            updateOne: {
                filter: { _id: new mongoose_1.default.Types.ObjectId(node.skillId), constellationMapId: map._id },
...
app.delete('/api/constellation-maps/:id', requireAdmin, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid constellation map ID' });
        }
        const map = await ConstellationMap_1.default.findById(req.params.id).select('_id');
        if (!map)
            return res.status(404).json({ error: 'Constellation map not found' });
        const cascade = req.query.cascade === 'true';
        if (!cascade) {
            await (0, constellationService_1.assertConstellationMapCanBeDeleted)(req.params.id);
            await map.deleteOne();
            invalidateConstellationMapCache();
        ...
        const ownedSkills = await Skill_1.default.find({ constellationMapId: { $in: mapIds } }).select('_id').lean();
        const deletedSkills = await deleteSkillsWithReferences(ownedSkills.map(skill => skill._id));
        const deletedMaps = await ConstellationMap_1.default.deleteMany({ _id: { $in: mapIds } });
        invalidateConstellationMapCache();
        res.json({
            success: true,
...
app.get('/api/skills', requireAuth, async (req, res) => {
    try {
        ...
        const allSkills = includeInactive
            ? await Skill_1.default.find({}).sort({ layer: 1, position: 1 }).lean()
...
app.post('/api/skills', requireConstellationSkillEditor, async (req, res) => {
    try {
        ...
        const questCount = await Skill_1.default.countDocuments();
        ...
        await (0, constellationService_1.validateSkillMapAssignment)({
...
app.put('/api/skills/:id', requireConstellationSkillEditor, async (req, res) => {
    try {
        ...
        if (cost !== undefined)
            skill.cost = cost;
        if (nextQuestCost !== undefined)
            skill.nextQuestCost = nextQuestCost;
        ...
        if (treePosition !== undefined)
            skill.treePosition = treePosition;
        if (constellationPosition !== undefined) {
        ...
        if (mapNodeRole !== undefined)
            skill.mapNodeRole = mapNodeRole;
        ...
        if (isAdvancedLocked !== undefined)
            skill.isAdvancedLocked = isAdvancedLocked === true;
        if (nodeColor !== undefined)
            skill.nodeColor = nodeColor;
        ...
        if (minAP !== undefined)
            skill.minAP = minAP !== null && minAP !== '' ? minAP : undefined;
        if (maxAP !== undefined)
            skill.maxAP = maxAP !== null && maxAP !== '' ? maxAP : undefined;
        ...
        if (connections !== undefined || constellationMapId !== undefined) {
            await assertValidConnectionTargets(skill._id.toString(), skill.constellationMapId, skill.connections || []);
        }
        await skill.save();
        invalidateSkillCaches();
        // Reload from database to ensure we return the latest data
        const updatedSkill = await Skill_1.default.findById(skill._id);
...
app.patch('/api/constellation-maps/:id/skills/batch', requireConstellationSkillEditor, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid constellation map ID' });
        }
        const skillIds = Array.isArray(req.body?.skillIds) ? [...new Set(req.body.skillIds.map(String))] : [];
        if (skillIds.length < 1 || skillIds.length > 500 || skillIds.some(id => !mongoose_1.default.Types.ObjectId.isValid(id))) {
        ...
        const map = await ConstellationMap_1.default.findById(req.params.id).select('_id scope').lean();
        if (!map)
            return res.status(404).json({ error: 'Constellation map not found' });
        const matchingCount = await Skill_1.default.countDocuments({
            _id: { $in: skillIds },
            constellationMapId: map._id
        });
        if (matchingCount !== skillIds.length) {
            throw new constellationService_1.ConstellationOperationError('Every selected star must belong to this constellation map', 409);
        }
        ...
        if (field === 'mapNodeRole') {
            if (!['topic-gateway', 'lesson', 'boss', 'capstone'].includes(value)) {
                throw new constellationService_1.ConstellationOperationError('Invalid star type');
            }
            (0, constellationService_1.assertRoleAllowedForScope)(map.scope, value);
        }
        else if (field === 'constellationLabel') {
            if (typeof value !== 'string' || value.length > 80) {
                throw new constellationService_1.ConstellationOperationError('Star label must be at most 80 characters');
            }
            value = value.trim() || undefined;
        }
        else if (typeof value !== 'boolean') {
            throw new constellationService_1.ConstellationOperationError(`${field} must be a boolean`);
        }
        const update = value === undefined ? { $unset: { [field]: 1 } } : { $set: { [field]: value } };
        await Skill_1.default.updateMany({ _id: { $in: skillIds }, constellationMapId: map._id }, update);
        invalidateSkillCaches();
        const updatedSkills = await Skill_1.default.find({ _id: { $in: skillIds } }).lean();
...
app.delete('/api/skills/:id', requireConstellationSkillEditor, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid quest ID' });
        }
        ...
        const cascade = req.query.cascade === 'true';
        const dependentTopic = await ConstellationMap_1.default.findOne({ gatewaySkillId: skill._id }).select('_id').lean();
        if (dependentTopic && !cascade)
            await (0, constellationService_1.assertSkillCanBeDeleted)(req.params.id);
        let deletedMaps = 0;
        const skillIds = [skill._id];
        let dependentMapIds = [];
        if (dependentTopic) {
            dependentMapIds = await collectConstellationMapIds(dependentTopic._id);
            const ownedSkills = await Skill_1.default.find({ constellationMapId: { $in: dependentMapIds } }).select('_id').lean();
        ...
        if (dependentMapIds.length > 0) {
            const mapResult = await ConstellationMap_1.default.deleteMany({ _id: { $in: dependentMapIds } });
...
app.post('/api/skills/:id/connections', requireConstellationSkillEditor, async (req, res) => {
    try {
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id) || !mongoose_1.default.Types.ObjectId.isValid(targetSkillId)) {
            return res.status(400).json({ error: 'Invalid quest ID' });
        }
        ...
        const existingConnection = skill.connections.find((conn) => conn.targetSkillId.toString() === targetSkillId);
...
app.put('/api/skills/:id/connections/:targetSkillId', requireConstellationSkillEditor, async (req, res) => {
    try {
        const { id, targetSkillId } = req.params;
        const { hasArrowhead, breakPoints, connectionType, curveMode, controlPoints } = req.body;
        if (!mongoose_1.default.Types.ObjectId.isValid(id) || !mongoose_1.default.Types.ObjectId.isValid(targetSkillId)) {
            return res.status(400).json({ error: 'Invalid quest ID' });
        }
        ...
        const connection = skill.connections.find((conn) => conn.targetSkillId.toString() === targetSkillId);
        if (!connection) {
        ...
        if (hasArrowhead !== undefined)
            connection.hasArrowhead = hasArrowhead;
        if (breakPoints !== undefined)
            connection.breakPoints = breakPoints;
        if (connectionType !== undefined)
            connection.connectionType = connectionType;
        if (curveMode !== undefined)
            connection.curveMode = curveMode;
...
app.delete('/api/skills/:id/connections/:targetSkillId', requireConstellationSkillEditor, async (req, res) => {
    try {
        const { id, targetSkillId } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id) || !mongoose_1.default.Types.ObjectId.isValid(targetSkillId)) {
            return res.status(400).json({ error: 'Invalid quest ID' });
        ...
        // Remove connection
        skill.connections = skill.connections.filter((conn) => conn.targetSkillId.toString() !== targetSkillId);
...
app.post('/api/skills/migrate-node-type', requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/skill-tree-settings', requireAuth, async (req, res) => {
    try {
        let settings = await SkillTreeSettings_1.default.findOne();
        if (!settings) {
            const defaultLayerGaps = new Map();
            for (let i = 1; i <= 7; i++) {
                defaultLayerGaps.set(String(i), 120); // Use string keys for Mongoose Map
            }
            settings = new SkillTreeSettings_1.default({
                layerGap: 120,
        ...
        if (settingsObj.layerGaps && settingsObj.layerGaps instanceof Map) {
            const gapsObj = {};
            settingsObj.layerGaps.forEach((value, key) => {
...
app.put('/api/skill-tree-settings', requireSuperAdmin, async (req, res) => {
    try {
        ...
        if (!settings) {
            ...
            if (layerGaps !== undefined) {
                ...
                settings.layerGaps = layerGapsMap;
            }
...
app.post('/api/skills/:id/unlock', requireAuth, async (req, res) => {
    try {
        const skillId = req.params.id;
        ...
        if (skill.prerequisites && skill.prerequisites.length > 0) {
            const missingPrerequisites = skill.prerequisites.filter((prereqId) => !unlockedSkills.includes(prereqId));
        ...
        const prerequisiteSkillsFromConnections = allSkills.filter((s) => s.connections && s.connections.some((conn) => conn.targetSkillId?.toString() === skillId));
        ...
        const updatedUser = await (0, progressionService_1.unlockSkillOnce)(userId, skillId, cost, isAdventure ? 25 : 0);
        if (!updatedUser) {
            const currentUser = await User_1.default.findOne({ discordId: userId }).select('assetPoints unlockedSkills').lean();
...
app.get('/api/user/unlocked-skills', requireAuth, async (req, res) => {
    try {
        const user = await User_1.default.findOne({ discordId: req.user.id }).select('unlockedSkills');
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json({
...
const getMissingQuestPrerequisites = async (skill, unlockedSkills) => {
    const missing = new Set((skill.prerequisites || []).filter((prerequisiteId) => !unlockedSkills.includes(prerequisiteId)));
    const connectedPrerequisites = await Skill_1.default.find({
    ...
    connectedPrerequisites.forEach(prerequisite => {
        ...
        if (!unlockedSkills.includes(prerequisiteId))
            missing.add(prerequisiteId);
    });
    return [...missing];
};
app.get('/api/user/quest-progress', requireAuth, async (req, res) => {
    try {
        const [user, pendingRequests] = await Promise.all([
            User_1.default.findOne({ discordId: req.user.id }).select('completedQuestSteps completedQuestRewards'),
            ApprovalRequest_1.default.find({ userId: req.user.id, status: 'pending' }).select('skillId')
        ]);
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        res.json({
...
app.post('/api/skills/:id/steps/:stepId/complete', requireAuth, async (req, res) => {
    try {
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: 'Invalid quest ID' });
        }
        const user = await User_1.default.findOne({ discordId: req.user.id });
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        ...
        const updatedUser = await (0, progressionService_1.completeQuestStepOnce)(req.user.id, skill.id, stepId, QUEST_STEP_REWARD_AP);
        if (!updatedUser) {
        ...
        const questCompleted = (0, progressionService_1.areQuestStepsComplete)(steps, completeStepIds);
        sessionUserCache.delete(req.user.id);
        res.json({
            success: true,
...
app.post('/api/skills/:id/approval-request', requireAuth, async (req, res) => {
    try {
        const skillId = req.params.id;
        const userId = req.user.id;
        ...
        const steps = skill.subQuests || [];
        const completedStepIds = new Set((user.completedQuestSteps || [])
            .filter(step => step.skillId === skillId)
            .map(step => step.stepId));
        const allStepsCompleted = (0, progressionService_1.areQuestStepsComplete)(steps, completedStepIds);
        ...
        const existingPendingRequest = await ApprovalRequest_1.default.findOne({
            userId,
            skillId,
            status: 'pending'
        });
        ...
        const approvedRequest = await ApprovalRequest_1.default.findOne({
            userId,
            skillId,
            status: 'approved'
        });
...
app.get('/api/approval-requests', requireAdmin, async (req, res) => {
    try {
        ...
        if (guildId && guildId !== 'all') {
            const usersInGuild = await User_1.default.find({ guildId: guildId }).select('discordId').lean();
            const userIds = usersInGuild.map(u => u.discordId);
            query.userId = { $in: userIds };
        }
        const requests = await ApprovalRequest_1.default.find(query)
            .sort({ createdAt: -1 })
            .lean();
            ...
            Skill_1.default.find({
                _id: {
                    $in: requests
                        ...
                        .filter(skillId => mongoose_1.default.Types.ObjectId.isValid(skillId))
                }
        ...
        const guildIds = [...new Set([...usersById.values()]
                .map(user => user.guildId)
                .filter((guildId) => Boolean(guildId) && mongoose_1.default.Types.ObjectId.isValid(guildId)))];
        const guilds = guildIds.length > 0
            ? await Guild_1.default.find({ _id: { $in: guildIds } }).select('name').lean()
            : [];
...
app.post('/api/approval-requests/:id/approve', requireAdmin, async (req, res) => {
    try {
        ...
        const approvalRequest = await ApprovalRequest_1.default.findById(requestId).select('userId skillId status').lean();
        if (!approvalRequest)
            return res.status(404).json({ error: 'Approval request not found' });
        if (approvalRequest.status !== 'pending') {
            return res.status(409).json({ error: 'This request has already been processed' });
        }
        const targetUser = await User_1.default.findOne({ discordId: approvalRequest.userId }).select('level').lean();
        if (!targetUser)
            return res.status(404).json({ error: 'User not found' });
        const eligibleSkill = mongoose_1.default.Types.ObjectId.isValid(approvalRequest.skillId)
        ...
        let approvalResult;
        try {
            approvalResult = await (0, approvalService_1.approveQuestRequest)(requestId, adminId, rewardAP);
        }
        catch (error) {
            if (error instanceof approvalService_1.ApprovalOperationError) {
...
app.get('/api/admin/images', requireSuperAdmin, async (req, res) => {
    try {
        ...
        const allSkills = await Skill_1.default.find({}).lean();
...
app.delete('/api/admin/images/:filename', requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/inventory', requireAuth, async (req, res) => {
    try {
        ...
        if (shouldRefresh && (0, hamsterQuest_1.isHamsterQuestConfigured)()) {
            try {
                await syncHamsterQuestInventory(userId, true);
            }
            catch (error) {
                syncWarning = (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error);
...
app.get('/api/inventory/hamsterquest/link-url', requireAuth, (req, res) => {
    const callbackUrl = `${FRONTEND_URL}/hamster-link`;
    res.json({ success: true, url: (0, hamsterQuest_1.getHamsterQuestLinkUrl)(callbackUrl) });
});
app.post('/api/inventory/hamsterquest/link', requireAuth, async (req, res) => {
    try {
        const token = typeof req.body?.token === 'string' ? req.body.token.trim() : '';
        if (!token) {
            return res.status(400).json({ error: 'HamsterQuest token is required' });
        }
        const profile = await (0, hamsterQuest_1.validateHamsterQuestToken)(token);
        ...
        await User_1.default.updateOne({ discordId: req.user.id }, { $set: { hamsterQuestAccessToken: token, hamsterQuestLinkedAt: new Date() } });
        sessionUserCache.delete(req.user.id);
        await syncHamsterQuestInventory(req.user.id, true);
        res.json({ success: true });
    }
    catch (error) {
        console.error('Error linking HamsterQuest account:', (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error));
        res.status((0, hamsterQuest_1.isHamsterQuestUnauthorized)(error) ? 401 : 502).json({
            error: (0, hamsterQuest_1.isHamsterQuestUnauthorized)(error)
                ? 'HamsterQuest login expired. Please try linking again.'
                : (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error)
        });
    }
});
app.post('/api/inventory/:purchaseId/use', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        if (!mongoose_1.default.Types.ObjectId.isValid(req.params.purchaseId)) {
        ...
        const shopItem = await ShopItem_1.default.findById(purchase.shopItemId);
        ...
        const catalogItem = await (0, officeCatalog_1.getOfficeCatalogItem)(shopItem.externalItemId);
        ...
        const user = await User_1.default.findOne({ discordId: userId }).select('+hamsterQuestAccessToken');
        if (!user?.hamsterQuestAccessToken) {
            return res.status(428).json({
                error: 'Link HamsterQuest before using this item',
                code: 'HAMSTERQUEST_LINK_REQUIRED',
                linkUrl: (0, hamsterQuest_1.getHamsterQuestLinkUrl)(`${FRONTEND_URL}/hamster-link`)
            });
        }
        const remoteInventory = await (0, hamsterQuest_1.getHamsterQuestInventory)(userId);
        ...
        let useResult;
        try {
            useResult = await (0, hamsterQuest_1.useHamsterQuestItem)(user.hamsterQuestAccessToken, remoteItem.inventoryItemId, 1);
        }
        catch (error) {
            if ((0, hamsterQuest_1.isHamsterQuestUnauthorized)(error)) {
        ...
        console.error('Error using inventory item:', (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error));
        res.status(502).json({ error: (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error) });
    }
});
...
app.get('/api/shop/items', requireAuth, async (req, res) => {
    try {
        ...
        const items = await ShopItem_1.default.find({ isActive: true, ...visibilityFilter }).sort({ createdAt: -1 });
        ...
        const purchases = await Purchase_1.default.find({ userId });
...
app.get('/api/admin/shop/items', requireAdmin, async (req, res) => {
    try {
        const items = await ShopItem_1.default.find({ isInventoryOnly: { $ne: true } }).sort({ createdAt: -1 });
...
app.get('/api/admin/office-catalog/items', requireAdmin, async (req, res) => {
    try {
        const [items, imports] = await Promise.all([
            (0, officeCatalog_1.getOfficeCatalogItems)(),
            ShopItem_1.default.find({ externalSource: 'office-catalog', isInventoryOnly: { $ne: true } }).select('externalItemId price isActive').lean()
        ]);
...
app.post('/api/admin/office-catalog/items/import', requireAdmin, async (req, res) => {
    try {
        ...
        const catalogItem = await (0, officeCatalog_1.getOfficeCatalogItem)(externalItemId);
        if (!catalogItem) {
            return res.status(404).json({ error: 'Office catalog item not found' });
        }
        const existingItem = await ShopItem_1.default.findOne({
            externalSource: 'office-catalog',
        ...
        const shopItem = await ShopItem_1.default.create({
...
app.post('/api/admin/shop/items', requireAdmin, async (req, res) => {
    try {
        ...
        const shopItem = new ShopItem_1.default({
            title,
...
app.put('/api/admin/shop/items/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const shopItem = await ShopItem_1.default.findById(req.params.id);
        if (!shopItem) {
        ...
        if (!shopItem.externalItemId) {
            ...
            if (imageUrl !== undefined)
                shopItem.imageUrl = imageUrl;
            if (itemType !== undefined)
                shopItem.itemType = itemType;
            if (productData !== undefined)
                shopItem.productData = productData;
        }
        if (price !== undefined) {
            if (typeof price !== 'number' || !Number.isFinite(price) || price < 0) {
                return res.status(400).json({ error: 'Price must be a non-negative number' });
...
app.delete('/api/admin/shop/items/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const shopItem = await (0, shopItemService_1.retireShopItem)(req.params.id);
        invalidateInventoryItemCache();
        res.json({ success: true, item: shopItem, message: 'Shop item retired successfully' });
    }
    catch (error) {
        if (error instanceof shopItemService_1.ShopItemOperationError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
app.get('/api/shop/items/:id/fiction', requireAuth, async (req, res) => {
    try {
        ...
        const shopItem = await ShopItem_1.default.findById(itemId);
        if (!shopItem) {
            return res.status(404).json({ error: 'Shop item not found' });
        }
        // Check if user is admin
        const currentUser = await User_1.default.findOne({ discordId: userId });
        ...
        const purchase = await Purchase_1.default.findOne({ userId, shopItemId: itemId });
        ...
        const contributions = await FictionContribution_1.default.find({ shopItemId: itemId })
            .sort({ createdAt: 1 })
            .lean();
        const usersById = await getUserSummariesByDiscordId(contributions.map(contribution => contribution.userId));
        const contributionsWithUserInfo = contributions.map((contribution) => {
...
app.get('/api/shop/items/:id/fiction/writing-lock', requireAuth, async (req, res) => {
    try {
        const itemId = req.params.id;
        const userId = req.user.id; // Discord ID
        // Validate ObjectId
        ...
        const now = new Date();
        const lockResult = await (0, fictionService_1.acquireFictionWritingLock)(itemId, userId);
        const lockOwner = lockResult.acquired
            ? currentUser
            : await User_1.default.findOne({ discordId: lockResult.lock.userId });
        return res.json({
            success: true,
...
app.post('/api/shop/items/:id/fiction/release-lock', requireAuth, async (req, res) => {
    try {
        const itemId = req.params.id;
        const userId = req.user.id; // Discord ID
        // Validate ObjectId
        ...
        const lock = await FictionWritingLock_1.default.findOne({ shopItemId: itemId, userId });
        if (lock) {
            await FictionWritingLock_1.default.deleteOne({ _id: lock._id });
        }
        res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/shop/items/:id/fiction/contribute', requireAuth, async (req, res) => {
    try {
        const itemId = req.params.id;
        const userId = req.user.id; // Discord ID
        ...
        let contribution;
        try {
            contribution = await (0, fictionService_1.contributeToFiction)({
                shopItemId: itemId,
                userId,
            ...
            if (error instanceof fictionService_1.FictionOperationError) {
...
app.get('/api/admin/shop/purchases', requireAdmin, async (req, res) => {
    try {
        const purchases = await Purchase_1.default.find({})
        ...
        const purchasesWithUserInfo = purchases.map((purchase) => {
...
app.get('/api/admin/shop/purchase-operations/pending', requireSuperAdmin, async (_req, res) => {
    try {
        const operations = await ExternalPurchaseOperation_1.default.find({ status: 'reserved' })
            .sort({ updatedAt: 1 })
            .limit(200)
            .lean();
...
app.post('/api/admin/shop/purchase-operations/:operationId/resolve', requireSuperAdmin, async (req, res) => {
    try {
        ...
        if (action === 'complete') {
            const operation = await (0, purchaseService_1.completeExternalPurchase)(operationId);
            if (!operation)
                return res.status(404).json({ error: 'Purchase operation not found' });
            return res.json({ success: true, operation });
        }
        const operation = await ExternalPurchaseOperation_1.default.findOne({ operationId }).lean();
        if (!operation)
            return res.status(404).json({ error: 'Purchase operation not found' });
        if (operation.status === 'completed') {
            return res.status(409).json({ error: 'A completed purchase cannot be refunded through reconciliation' });
        }
        const refunded = await (0, purchaseService_1.rollbackExternalPurchase)(operationId);
...
app.get('/api/admin/shop/items/:id/purchases', requireAdmin, async (req, res) => {
    try {
        const itemId = req.params.id;
        // Validate ObjectId
        if (!mongoose_1.default.Types.ObjectId.isValid(itemId)) {
        ...
        const purchases = await Purchase_1.default.find({ shopItemId: itemId })
            .sort({ createdAt: -1 })
            .lean();
        const usersById = await getUserSummariesByDiscordId(purchases.map(purchase => purchase.userId));
        const purchasesWithUserInfo = purchases.map((purchase) => {
...
app.post('/api/shop/items/:id/purchase', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id; // Discord ID
        const itemId = req.params.id;
        if (!mongoose_1.default.Types.ObjectId.isValid(itemId)) {
        ...
        if (shopItem.externalSource === 'office-catalog' && shopItem.externalItemId) {
            externalItem = await (0, officeCatalog_1.getOfficeCatalogItem)(shopItem.externalItemId);
            if (!externalItem) {
        ...
        try {
            if (isExternalInventoryItem && shopItem.externalItemId) {
                ...
                purchaseResult = await (0, purchaseService_1.reserveExternalPurchase)({
                ...
                purchaseResult = await (0, purchaseService_1.reserveLocalPurchase)(purchaseInput);
            }
        }
        catch (error) {
            if (error instanceof purchaseService_1.PurchaseOperationError) {
                return res.status(error.status).json({ error: error.message, code: error.code });
            }
            throw error;
        }
        if (isExternalInventoryItem && shopItem.externalItemId) {
            try {
                await (0, hamsterQuest_1.grantHamsterQuestItem)(userId, shopItem.externalItemId, 1, externalOperationId || undefined);
                await (0, purchaseService_1.completeExternalPurchase)(externalOperationId);
            }
            catch (error) {
                const definiteRejection = axios_1.default.isAxiosError(error) && Boolean(error.response) &&
                    error.response.status >= 400 && error.response.status < 500;
                if (definiteRejection) {
                    await (0, purchaseService_1.rollbackExternalPurchase)(externalOperationId);
                    return res.status(502).json({
                        error: `HamsterQuest rejected this item: ${(0, hamsterQuest_1.getHamsterQuestErrorMessage)(error)}`,
            ...
            try {
                ...
                console.error(`Purchased item was granted but inventory sync failed for ${userId}:`, (0, hamsterQuest_1.getHamsterQuestErrorMessage)(error));
            }
        }
...
app.post('/api/admin/reset-skills-by-nickname', requireSuperAdmin, async (req, res) => {
    try {
        ...
        const result = await User_1.default.updateMany({ nickname: { $regex: `^${nicknamePrefix}`, $options: 'i' } }, { $set: { unlockedSkills: [] } });
        invalidateProgressionCache();
        res.json({
            success: true,
...
app.get('/api/leaderboard/progression', requireAuth, async (req, res) => {
    try {
        const [currentUser, snapshot] = await Promise.all([
            User_1.default.findOne({ discordId: req.user.id }).select('guildId').lean(),
